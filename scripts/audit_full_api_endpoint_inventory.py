...
def main() -> int:
    ...
    parser.add_argument(
        "--install",
        action="store_true",
        help="Explicitly install the inventory metadata into DuckDB; default is read-only audit.",
    )
    ...
    inserted = (
        install_endpoint_inventory(
            ...
            items,
            api_base=API_BASE,
            probe_date=selected_date,
        )
        if args.install
        else 0
    )
    report = render_inventory_report(items, date=selected_date, stock_code=stock_code, sector_code=sector_code)
    ...
    print(f"api_endpoint_inventory_rows={inserted} installed={str(args.install).lower()}")
    print(f"api_endpoint_inventory_report={out}")
    print(f"api_endpoint_inventory_docs={docs_out}")
    print(f"date={selected_date} stock_code={stock_code} sector_code={sector_code} live_probe={str(not args.no_live).lower()}")
