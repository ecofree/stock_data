...
def _refresh_default_concept_views(db: duckdb.DuckDBPyConnection) -> None:
    """Refresh the quality-gated THS concept relations.

    These views are part of the canonical review contract, so maintenance
    commands that rebuild normalized views must refresh them as well.  A
    ...
    db.execute("""
        ...
        WITH concept_quality AS (
            SELECT trade_date, count(DISTINCT concept_code) AS concept_count,
                   ...
                   sum(CASE WHEN coalesce(date_verified,false) THEN 1 ELSE 0 END) AS verified_concepts,
                   count(DISTINCT CASE WHEN json_valid(raw_json)
                       THEN json_extract_string(raw_json,'$.fetched_date') END) AS fetched_dates,
                   sum(CASE WHEN coalesce(stock_count,0)=0 THEN 1 ELSE 0 END) AS zero_concepts
            FROM ths_concept_daily GROUP BY trade_date
        ), member_quality AS (
            SELECT trade_date, count(*) AS member_rows,
                   sum(CASE WHEN coalesce(date_verified,false) THEN 1 ELSE 0 END) AS verified_members,
                   count(DISTINCT CASE WHEN json_valid(raw_json)
                       THEN json_extract_string(raw_json,'$.fetched_date') END) AS fetched_dates
            FROM ths_concept_stock_history GROUP BY trade_date
        ), checkpoint_quality AS (
            ...
            SELECT t.trade_date, t.concept_code, t.concept_name, t.rank,
                   t.stock_count, t.source, t.raw_json, t.date_verified
            FROM ths_concept_daily t
            JOIN provider_choice p
              ON p.trade_date = t.trade_date
             AND p.concept_code = t.concept_code
             AND p.provider_rank = 1
            JOIN snapshot_quality q ON q.trade_date=t.trade_date
            WHERE coalesce(
    ...
    db.execute("""
        ...
        WITH concept_quality AS (
            ...
            SELECT h.trade_date, h.concept_code, h.concept_name,
                   h.stock_code, h.stock_name, h.concept_rank,
                   h.source, h.raw_json, h.date_verified
            FROM ths_concept_stock_history h
            JOIN provider_choice p
              ON p.trade_date = h.trade_date
             AND p.concept_code = h.concept_code
             AND p.provider_rank = 1
            JOIN snapshot_quality q ON q.trade_date=h.trade_date
            WHERE coalesce(
...
def init_schema(db: duckdb.DuckDBPyConnection):
    ...
    # semantics behind.
    _refresh_default_concept_views(db)
    ...
    db.execute("""
        CREATE TABLE IF NOT EXISTS close_snapshot_certification (
            dataset VARCHAR NOT NULL,
            trade_date DATE NOT NULL,
            provider VARCHAR NOT NULL DEFAULT 'tushare',
            expected_rows INTEGER DEFAULT 0,
            observed_rows INTEGER DEFAULT 0,
            distinct_codes INTEGER DEFAULT 0,
            ...
            PRIMARY KEY(dataset, trade_date, provider)
        )
    """)
    ...
    # Versioned forward-only migrations under migrations/ are applied exactly
    # once per database; ad-hoc ALTERs above remain only for pre-migration
    # compatibility shims that have not been converted yet.
