...
def export_projection(data):
    ...
    if 'plans' in result:
        ...
        for plan in result['plans'].get('rows',[]):
            for key in ('account_id','account_snapshot_id'):plan.pop(key,None)
        result['plans']['scope']='redacted_display_not_authoritative_plan_receipts'
    result.pop('report_id',None);result['report_id']=identity(result)
    return result
...
<p id="note-context" class="small"></p><form method="post" action="/note" id="note-form"><input type="hidden" name="csrf" value="__CSRF__"><input type="hidden" name="request_id" id="request-id"><input type="hidden" name="prediction_id" id="prediction-id"><input type="hidden" name="supersedes" id="supersedes"><input type="hidden" name="evidence_id" id="evidence-id">
<div class="layout"><div><label>证券 <input name="instrument" id="instrument" list="security-options" pattern="[0-9]{6}" maxlength="6" required><datalist id="security-options"></datalist></label><label>署名 <input name="operator" required maxlength="200"></label><label>意向 <select name="intent"><option value="observe">继续观察</option><option value="reject">不采用</option><option value="paper_hypothesis">研究假设（不下单）</option></select></label></div><div><label>判断依据 <textarea name="hypothesis" required maxlength="4000"></textarea></label><label>失效条件 <textarea name="invalidation" required maxlength="4000"></textarea></label></div></div><button id="save">保存判断到本地</button> <button type="button" id="new-note">退出修订，记录当前判断</button></form><div id="notes"></div></section>
...
rows.forEach(r=>{const o=el('option',r.instrument);o.value=r.instrument;$('security-options').append(o)});$('prediction-id').value=D.market?'':P?P.prediction_id:'';$('evidence-id').value=D.market?.snapshot_id||'';$('save').disabled=!D.market&&!P;
...
function resetNote(){draftDirty=false;$('note-form').reset();$('request-id').value=newRequest();$('prediction-id').value=D.market?'':P?P.prediction_id:'';$('evidence-id').value=D.market?.snapshot_id||'';$('supersedes').value='';$('note-context').textContent='绑定证据 '+(D.market?.snapshot_id||P?.prediction_id||'未配置市场').slice(0,12)+'；以保存时刻区分事前与事后，不可回填时间。'}resetNote();$('new-note').onclick=()=>{if(!draftDirty||confirm('放弃尚未保存的草稿？'))resetNote()};
D.notes.forEach(n=>{const e=el('div','');e.className='note';e.append(el('strong',n.instrument+' · '+n.operator+' · '+n.received_at),el('p',(timingNames[n.timing]||'旧记录，时间待核对')+' / '+(n.is_latest?'当前记录':'历史版本')+' / '+(n.prediction_id||n.evidence_id||'').slice(0,12)),el('p',n.hypothesis),el('p','失效：'+n.invalidation));if(n.is_latest){const b=el('button','追加修订，保留原文');b.type='button';b.onclick=()=>{const form=$('note-form');for(const k of ['operator','hypothesis','invalidation','intent'])form.elements[k].value=n[k];$('instrument').value=n.instrument;$('prediction-id').value=n.prediction_id||'';$('evidence-id').value=n.evidence_id||'';$('supersedes').value=n.note_id;$('note-context').textContent='修订 '+n.note_id.slice(0,12)+'，仍绑定原预测；事后修订不替代事前判断。';$('journal').scrollIntoView()};e.append(b)}$('notes').append(e)});
...
function themes(){
 ...
 if(modern){$('theme-workspace').querySelector('summary').textContent='题材完整成员与研究覆盖';const h=$('theme-workspace').querySelectorAll('th');h[1].textContent='涨停 / 全成员';h[2].textContent='最高连板';}
 table($('theme-rows'),all.map(t=>[t.concept_name,modern?(t.limit_up_count??'—')+' / '+t.member_count:(t.limit_up_count??'—')+' / '+t.member_count,modern?((t.max_board??'—')+(t.broad_classification?' · 宽泛分类':'')):(t.max_board??'—')]));
 [...$('theme-rows').rows].forEach((tr,i)=>{tr.tabIndex=0;tr.onclick=()=>{
   ...
   function draw(){const filtered=members.filter(m=>((m.stock_name||'')+' '+m.stock_code).includes(search.value.trim()));list.replaceChildren();
     for(const member of filtered.slice(0,limit)){const p=el('p',(member.stock_name||'')+' '+member.stock_code+(modern?' / 涨跌 '+fmt(member.change_pct)+'%':' / '+member.board_level+'板'));const r=rows.find(r=>r.instrument===member.stock_code);
       if(r){const b=el('button',r.market_only?'查看市场证据':'查看冻结研究证据');b.type='button';b.onclick=()=>{show(r);$('today').scrollIntoView()};p.append(b)}else p.append(el('span',' · 不在冻结研究池，不生成模型分数'));list.append(p)}
...
const ecology=el('details','');ecology.append(el('summary','连板梯队、昨日队列与题材延续'));
ecology.append(el('p','最高连板 '+(market.highest_board??'缺失')+'；成交额 '+(market.turnover?.cny==null?'缺失':fmt(market.turnover.cny/1e8)+'亿元')+'（可用标准价格覆盖，不冒充交易所总量）'));
for(const step of market.limit_pool?.ladder||[])ecology.append(el('p',step.height+' 板：'+step.count+' 只'));
for(const row of (market.previous_limit_queue?.rows||[]).slice(0,100))ecology.append(el('p','昨日池 '+row.stock_code+' '+row.stock_name+' / '+row.board_level+'板 → 当日涨跌 '+fmt(row.next_change_pct)+'%'));
if(market.concept_flow)ecology.append(el('p','题材资金采用同日18:00回看窗口；成员/来源一致的历史对照状态：'+market.concept_flow.history.status+'。不作为原时点可得性或选股优势证明。'));
$('market').append(ecology);
}else{$('market-state').textContent='未附同日市场复盘快照；当前仅有下方冻结研究范围，不用局部证券推断全市场。';$('theme-workspace').hidden=true}
function appendMarketEvidence(box,r){
 const stock=D.market?.stocks?.[r.instrument];
 if(!stock){box.append(el('p','市场证据缺失；不能按模型分数补齐。'));return}
 box.append(el('p','同日价格事实：收盘 '+fmt(stock.close)+' 元 / 涨跌 '+fmt(stock.change_pct)+'%'));
 const themes=(D.market.themes||[]).filter(t=>!t.broad_classification&&(t.member_codes||[]).includes(r.instrument)).slice(0,3);
 for(const t of themes)box.append(el('p',t.concept_name+'：涨停 '+(t.limit_up_count??'缺失')+' / 成员 '+t.member_count+'，最高 '+(t.max_board??'缺失')+' 板；主力净额 '+(t.main_flow_cny==null?'未通过成员资金核对':fmt(t.main_flow_cny/1e8)+'亿元')));
 box.append(el('p','反对/风险证据：'+(stock.change_pct<0?'当日价格下跌；':'当日上涨不能证明次日延续；')+(themes.some(t=>t.main_flow_cny<0)?'相关题材主力净额为负；':'')+'题材成员是留存版本，资金缺失不代表流出。下一条件由你明确记录。'));
}
...
appendMarketEvidence(box,r);if(r.market_only){box.append(el('p','本次比较不依赖模型。模型窗口与贡献未核对，不把缺失显示为零缺口。'));$('compare-panel').append(box);continue}const contributions=Object.entries(r.contributions||{}).filter(([k])=>k!=='base_value');
...
function persistDraft(){draftDirty=true;const values={};for(const k of ['request_id','prediction_id','evidence_id','supersedes','instrument','operator','intent','hypothesis','invalidation'])values[k]=form.elements[k].value;try{sessionStorage.setItem(draftKey,JSON.stringify(values))}catch(_){$('message').textContent='浏览器不能保存草稿；离开前请先提交并取得回执。'}}
...
window.addEventListener('pageshow',()=>{$('save').disabled=!D.market&&!P;$('save').textContent='保存判断到本地';if(form.elements.hypothesis.value)draftDirty=true});
...
if(account.snapshot_id){
 plansPanel.append(el('p','账户快照 '+account.as_of+'，有效至 '+account.valid_until+'；现金 '+fmt(account.cash_fen/100)+' 元 / 冻结 '+fmt(account.frozen_fen/100)+' 元。'+(account.blockers||[]).join('；')));
 const holdings=el('table',''),body=el('tbody','');holdings.append(body);table(body,[['持仓','数量','可卖','标记价格'],...(account.positions||[]).map(p=>[p.instrument,p.quantity,p.sellable,p.mark_price])]);plansPanel.append(holdings);
 for(const order of account.open_orders||[])plansPanel.append(el('p','未完成委托 '+order.order_id+' / '+order.instrument+' / 剩余 '+order.remaining_quantity+' / 冻结 '+order.frozen_cash+' 元；须对账，不自动解除。'));
}
for(const p of D.plans?.rows||[]){const item=el('article','');item.className='note';item.append(el('strong',p.instrument+' · '+(p.is_latest?'当前计划':'历史版本')),el('p','条件 '+p.condition+' '+(p.threshold||'人工核对')+'；有效期 '+p.valid_until),el('p','状态 '+p.evaluation.state+'；失效：'+p.invalidation));if(online){const a=el('a','查看计划回执');a.href='/plan-receipt/'+p.plan_id;item.append(a)}plansPanel.append(item)}
$('observation').append(plansPanel);
if(online)for(const n of D.notes.filter(n=>n.is_latest&&n.intent==='observe')){
 const details=el('details','');details.append(el('summary','为 '+n.instrument+' 保存观察计划'));
 const f=document.createElement('form');f.method='post';f.action='/plan';
 for(const [k,v] of Object.entries({csrf:form.elements.csrf.value,note_id:n.note_id,operator:n.operator,request_id:newRequest(),supersedes:(D.plans?.rows||[]).find(p=>p.note_id===n.note_id&&p.is_latest)?.plan_id||''})){const input=document.createElement('input');input.type='hidden';input.name=k;input.value=v;f.append(input)}
 const label=el('label','触发条件'),select=document.createElement('select');select.name='condition';for(const [k,v] of [['manual','人工核对原判断条件'],['price_above','未复权价格达到或高于'],['price_below','未复权价格达到或低于']])select.append(new Option(v,k));label.append(select);f.append(label);
 for(const [k,name,type] of [['threshold','价格阈值（元，人工条件可留空）','number'],['valid_until','有效期（北京时间，例如 2026-09-14T15:00:00+08:00）','text']]){const label=el('label',name),input=document.createElement('input');input.name=k;input.type=type;if(k==='valid_until')input.required=true;else{input.step='0.01';input.min='0.01'}label.append(input);f.append(label)}
 const key='stock-data-plan-draft:'+n.note_id,keep=()=>{const value=Object.fromEntries(new FormData(f));delete value.csrf;try{sessionStorage.setItem(key,JSON.stringify(value))}catch(_){}};
 try{const saved=JSON.parse(sessionStorage.getItem(key)||'null');if(saved&&!D.plans?.rows.some(p=>p.request_id===saved.request_id)){for(const [k,v] of Object.entries(saved))if(f.elements[k])f.elements[k].value=v;draftDirty=true}else sessionStorage.removeItem(key)}catch(_){}
 const button=el('button','保存观察计划');f.append(button);
 f.addEventListener('input',()=>{f.elements.request_id.value=newRequest();keep();draftDirty=true});f.addEventListener('submit',()=>{keep();draftDirty=false;button.disabled=true});details.append(f);plansPanel.append(details);
}
...
def plan_receipt_page(plan):
    return ('<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>观察计划已保存</title>'+STYLE+
        '<main><h1>观察计划已保存</h1><p>不是委托，也未冻结账户资金。</p><dl>'+
        ''.join('<dt>'+escape(k)+'</dt><dd>'+escape(str(v))+'</dd>' for k,v in [
            ('回执',plan['plan_id']),('证券',plan['instrument']),('原判断',plan['note_id']),
            ('条件',plan['condition']),('阈值',plan['threshold']),('有效期',plan['valid_until']),('账户状态',plan['account_status'])])+
        '</dl><a href="/#observation">返回观察与计划</a></main></html>')
