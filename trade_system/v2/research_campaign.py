...
from .research_receipts import API_FIELDS, rows_for
...
FIELDS={**API_FIELDS,'trade_cal':['exchange','cal_date','is_open','pretrade_date'],
    ...
    for provider in ('hithink_native','xiaodefa_relay'):
        for code in codes:
            for start,end in windows:
                apis=[PRICES] if provider=='hithink_native' else ['daily','adj_factor','moneyflow']
                for api in apis:
...
def parse(r,data):
    if r['kind']=='security_history':
        ...
        return rows_for(r,data,max_items=max(64,days+1))
...
def capture(output,*,client=None,config=None,base=None,replay_cache=None):
    requests=plan(config)
    lineage={}
    if base is not None:
        base=Path(base).resolve()
        prior,members,_,_=cached_replay(base,replay_cache)
        depth=prior.get('incremental_depth',0)+1
        if depth>6 or config is None or prior.get('config') is None:
            raise ValueError('incremental lineage requires a configured base and at most six deltas')
        old=prior['config']
        if (old['codes']!=config['codes'] or old['selection_scope']!=config['selection_scope']
            or not old['start']<=config['start']<=old['end']<=config['end']
            or (date.fromisoformat(config['end'])-date.fromisoformat(config['start'])).days>14):
            raise ValueError('same universe and bounded overlapping revision window required')
        if prior['origin']!='native_and_relay' and client is None:
            raise ValueError('synthetic lineage cannot become live evidence')
        lineage={'base_folder':str(base),'base_manifest_id':identity(members),'incremental_depth':depth}
...
def cached_replay(folder,cache=None):
    """One parse per input per application run; hashes are still rechecked."""
    if cache is None:return replay(folder)
    key=str(Path(folder).resolve())
    if key in cache:
        if sealed(folder)!=cache[key][1]:raise ValueError('receipt package changed during run')
        return cache[key]
    cache[key]=replay(folder)
    return cache[key]


def derive(folder, *, replayed=None, replay_cache=None, _depth=0):
    if _depth>6:raise ValueError('incremental lineage exceeds bounded checkpoint window')
    reg,members,data,statuses=replayed if replayed is not None else cached_replay(folder,replay_cache)
    ...
    for code in configuration(reg.get('config'))[0]:
        ...
        for day in days[exchange]:
            ...
            output.append(row)
    if reg.get('base_folder'):
        parent=Path(reg['base_folder']).resolve()
        parent_replay=cached_replay(parent,replay_cache)
        if identity(parent_replay[1])!=reg.get('base_manifest_id'):
            raise ValueError('incremental base manifest changed')
        before=derive(parent,replayed=parent_replay,replay_cache=replay_cache,_depth=_depth+1)
        cfg=reg['config']; previous=parent_replay[0].get('config') or {}
        if (reg.get('incremental_depth')!=parent_replay[0].get('incremental_depth',0)+1
            or reg['incremental_depth']>6 or previous.get('codes')!=cfg['codes']
            or previous.get('selection_scope')!=cfg['selection_scope']
            or not previous['start']<=cfg['start']<=previous['end']<=cfg['end']
            or (date.fromisoformat(cfg['end'])-date.fromisoformat(cfg['start'])).days>14):
            raise ValueError('invalid incremental base/range binding')
        cutoff=(date.fromisoformat(cfg['end'])-timedelta(days=89)).isoformat()
        for exchange in days:
            days[exchange]=[d for d in before['calendar'][exchange] if cutoff<=d<cfg['start']]+days[exchange]
        oldrows=[r for r in before['rows'] if cutoff<=r['datetime']<cfg['start']]
        # Preserve each original receipt identity; never manufacture a combined raw response.
        for r in oldrows:
            r['receipt_files']={k:str(parent/v) if v and not Path(v).is_absolute() else v for k,v in r['receipt_files'].items()}
        output=oldrows+output
    bykey={(r['ts_code'],r['datetime']):r for r in output}
    for r in output:
        r['adjusted_price_target_ret']=None;r['adjusted_price_target_date']=None
