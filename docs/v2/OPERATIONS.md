...
2026-09-11 本轮候选版本 **0.3.2**，见 [七条工作线集中实施](SEVEN_WORKSTREAMS_DELIVERY_20260911.md)。每日复盘新增判断分组、原假设/风险及日内区间；可从封存恢复包离线重放后生成新页面，不覆写旧发布。原生全年补证中的 69 个量/额冲突继续隔离，禁止按猜测倍率入库。当前仍未迁移现网任务或放行真实执行。
...
2026-09-11 0.3.1 最新边界见 [按序工作包记录](NEXT_WORK_PACKAGES_20260911.md)。归档读校验最多 4 个只读线程，DuckDB 和状态重放仍单 owner；命令预算为安全边界检查，不是强杀或 I/O 硬实时截止。显式全审计失败后不再复用原热缓存。历史日历覆盖通过封存补证在临时表消费，旧库不改写；研究样本清单不能授予拟合权限。

2026-09-11 0.3.0 增量：见 [热状态与原生消费操作边界](HOT_NATIVE_ALPHA_20260911.md)。新纸面账户可明确声明 `bounded_hot_v3` 和活动容量，旧账户不自动迁移。冷启动仍需全历史审计预算；热状态小不代表恒定恢复耗时或恒定进程内存。降级到不认识新格式的旧包不是可用回退方案。`stock-data-daily enrich/enrichment-view` 可获取/展示受限原生来源；收盘前不伪造次日完成，真人与真实账户验收继续开放。

2026-09-11 补充：[ed47 六项实施记录](ED47_FOLLOWUP_20260911.md) 优先约束本轮候选发行。0.2.0 使用增量检查点，0.1.0 不可直接读取新载荷作为回退；必须保留原账本并使用同格式兼容、源码绑定验收通过的核心包向前恢复。禁止把旧 stock_data 任务配置、旧信号/通知/人工计划权限恢复为回滚方案。当前没有执行或批准生产切换。
