...
from trade_system.v2.research_semantics import (OBSERVED_PRICE_FIELDS as FIELDS,
    OBSERVED_UNIT_POLICY, qualify_observed_price as qualify)

POLICY = dict(OBSERVED_UNIT_POLICY, stock_code='302132')


def payload(receipts, db):
    receipts, db = Path(receipts).resolve(strict=True), Path(db).resolve(strict=True)
    code_hashes = {p.name: file_hash(p) for p in [Path(__file__), Path(probe.__file__), Path(semantics.__file__)]}
    ...
    if code_hashes != {p.name: file_hash(p) for p in [Path(__file__), Path(probe.__file__), Path(semantics.__file__)]}:
